...
public class Task_4_VacinationCenter {
    ...
    private static Task_4_Booth[] ServiceCenter = new Task_4_Booth[6];
    private static String[] boothObjectsOrdered = new String[ServiceCenter.length];
    public static LinkedList<Task_4_Booth> AstraZenecaWaitingList = new LinkedList<Task_4_Booth>();
    public static LinkedList<Task_4_Booth> SinopharmWaitingList = new LinkedList<Task_4_Booth>();
    public static LinkedList<Task_4_Booth> PfizerWaitingList = new LinkedList<Task_4_Booth>();
    ...
    private static void initialise() {
        for (int x = 0; x < ServiceCenter.length; x++ ){
            ServiceCenter[x] = new Task_4_Booth("empty","empty","empty","empty","empty",0);
        }
        System.out.println( "initialised");
    }
    ...
    private static void addPatientToBooth() throws IOException {
        ...
        if(vacType == 1){
            if(ServiceCenter[0].getFname() == "empty"){
                ...
                System.out.println("All booths for the requested vaccine are occupied. Added to waiting queue.");
                Task_4_Booth Wpatient = new Task_4_Booth(firstName,surname,age,city,id,1);
                AstraZenecaWaitingList.add(Wpatient);
                //addPatientToBooth();
            }
            ...
            if(ServiceCenter[2].getFname() == "empty"){
                ...
                System.out.println("All booths for the requested vaccine are occupied. Added to waiting queue.");
                Task_4_Booth Wpatient = new Task_4_Booth(firstName,surname,age,city,id,1);
                SinopharmWaitingList.add(Wpatient);
                addPatientToBooth();
            }
        }else if (vacType == 3){
            if(ServiceCenter[4].getsName() == "empty"){
                ...
                System.out.println("All booths for the requested vaccine are occupied. Added to waiting queue.");
                Task_4_Booth Wpatient = new Task_4_Booth(firstName,surname,age,city,id,1);
                PfizerWaitingList.add(Wpatient);
                addPatientToBooth();
            }
        }
    ...
    private static void removePatientFromBooth() throws IOException {
        ...
        if(boothNum == 0 || boothNum == 1){
            Task_4_Booth Wpatient = AstraZenecaWaitingList.getFirst();

            ServiceCenter[boothNum].setFname(Wpatient.getFname());
            ServiceCenter[boothNum].setsName(Wpatient.getsName());
            ServiceCenter[boothNum].setAge(Wpatient.getAge());
            ServiceCenter[boothNum].setCity(Wpatient.getCity());
            ServiceCenter[boothNum].setNic(Wpatient.getNic());

        }else if(boothNum == 2 || boothNum == 3){

            Task_4_Booth Wpatient = SinopharmWaitingList.getFirst();
            ...
            Task_4_Booth Wpatient = PfizerWaitingList.getFirst();
